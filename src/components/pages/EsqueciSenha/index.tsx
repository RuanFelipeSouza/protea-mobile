...
import { pacienteSolicitarCodigo, pacienteRedefinirSenha } from '../../../services/pacienteLoginService'
import { profSolicitarCodigo, profRedefinirSenha } from '../../../services/loginService'
...
/**
 * Recuperação de senha — mesma tela pra paciente/responsável e profissional,
 * só troca qual serviço chama e onde salva a sessão no final. O fluxo de
 * paciente já bate num backend real; o de profissional ainda não existe
 * (ver `profSolicitarCodigo`/`profRedefinirSenha` em `loginService.ts`) —
 * front pronto pra quando o back sair.
 */
export function EsqueciSenhaPage({ mode }: Props) {
  ...
  async function handleEnviarCodigo() {
    ...
    try {
      if (mode === 'paciente') {
        await pacienteSolicitarCodigo(email.trim(), 'recuperar_senha')
      } else {
        await profSolicitarCodigo(email.trim())
      }
      setStep('codigo')
      Alert.alert(
        'Código enviado',
  ...
  // Etapa 2 — valida o código, troca a senha e entra.
  async function handleRedefinir() {
    if (!email.trim() || !codigoOk || !allRulesOk || !senhasIguais) return
    setLoading(true)
    try {
      if (mode === 'paciente') {
        const result = await pacienteRedefinirSenha(email.trim(), codigo.trim(), senha)
        ...
        const result = await profRedefinirSenha(email.trim(), codigo.trim(), senha)
        if (!result.user) throw new Error('Resposta do servidor incompleta')
    ...
    >
      ...
      >
        ...
        <Text style={[styles.subtitle, { color: colors.textMuted }]}>
          {step === 'email'
            ? 'Informe o e-mail cadastrado na clínica. Enviaremos um código para você redefinir sua senha.'
            : 'Digite o código que enviamos para o seu e-mail e defina uma nova senha.'}
        </Text>
